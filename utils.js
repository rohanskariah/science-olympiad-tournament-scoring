...
function getTournamentNameParsed() {
  ...
  if (tournamentName == "" || tournamentName == "Tournament Name") {
    tournamentName = showPrompt("You have not entered a tournament name. Please enter one now")
  ...
  var parsedDate = Utilities.formatDate(tournamentDate, 'America/Los_Angeles', "d-MMMM-YYYY")
  if (parsedDate == "1-January-2000") {
    tournamentDate = showPrompt("You have not entered a tournament date. Please enter one now")
  ...
  if (tournamentDivision == "" || tournamentDivision == "__") {
    tournamentDivision = showPrompt("You have not entered a tournament division. Please enter one now")
  ...
  if (tournamentLocation == "" || tournamentLocation == "School_Name") {
    tournamentLocation = showPrompt("You have not entered a tournament location. Please enter one now")
    currentSheet.getRangeByName("Location").setValue(tournamentLocation)
  }

  var fullTournamentDate = parsedDate + " " + tournamentName + " Division-" + tournamentDivision + " @ " + tournamentLocation
...
function moveRows(templateSheet, newSheet, eventName) {
  ...
  for (let i = 0; i < replacementRanges.length; i++) {
    ...
    newSheet.getRange(range).setValues(templateSheet.getRange(range).getValues());
  }
}